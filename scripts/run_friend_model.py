"""
Run friend's trained model and record 5 laps of telemetry data.

Usage:
    conda run -n ai_env python scripts/run_friend_model.py [--host localhost] [--port 3001]

The model must be pre-trained:
    conda run -n ai_env python scripts/train_friend_model.py --csv data/<telemetry>.csv
"""
...
from external_models.friend_model import snakeoil3_jm2 as snakeoil3
...
def main(host: str = "localhost", port: int = 3001):
    model_dir = Path(__file__).resolve().parent.parent / "external_models" / "friend_model"
    model_path = model_dir / "driving_model.pt"
    scaler_path = model_dir / "driving_scaler.pkl"

    if not model_path.exists():
        print(f"[ERROR] Model not found: {model_path}")
        print("Run: conda run -n ai_env python scripts/train_friend_model.py --csv data/<telemetry>.csv")
        sys.exit(1)
